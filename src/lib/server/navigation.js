...
var lat = Infinity,
	long = Infinity; //Unreachable default values
var targetLat = Infinity,
	targetLong = Infinity;
var pixelX = 0,
	pixelY = 0;
var targetPixelX = 0,
...
const outgoing = {
	//Outgoing messages to web-clients
...
function readNavigationFile() {
	fs.readFile(`${global.path}/src/backend/data/navigation.json`, (error, data) => {
		if (error) {
			console.log(`Navigationdata could not be loaded: ${error}`);
			return;
		}
...
function setNavigation(location) {
	...
	sendAllClients(
		`${outgoing.set_navigation}:${global.navigation}`,
		`Navigation mit dem Wert "${global.navigation}" wird an alle Klienten gesendet.`,
		null,
		importance.LOW
	);
	sendAllClients(
		`${outgoing.coords}:${global.coords}`,
		`Navigation mit dem Wert "${global.coords}" wird an alle Klienten gesendet.`,
		null,
		importance.LOW
	);
...
function getNavigation(client) {
	send(
		client,
		`${outgoing.set_navigation}:${global.navigation}`,
		`Navigation mit dem Wert "${global.navigation}" wird zum Klienten gesendet.`,
		importance.LOW
	);
	send(
		client,
		`${outgoing.coords}:${global.coords}`,
		`Navigation mit dem Wert "${global.coords}" wird zum Klienten gesendet.`,
		importance.LOW
	);
}
...
function getDMSCoords(lat, long) {
	if (lat == Infinity || long == Infinity) return 'Lokalisieren...';
	else return `${decimalToDMS(lat, true)} ${decimalToDMS(long, false)}`;
}
...
function getLocationImage() {
	if (currentTile == null || !currentTileContainsLocation()) currentTile = getNewLocationObj();
...
function currentTileContainsLocation() {
	...
	if (
		!(
			currentTile.startLong <= long + stats.widthPixelLong &&
			currentTile.endLong >= long - stats.widthPixelLong
		)
	)
		return false;
	if (
		!(
			currentTile.endLat <= lat - stats.widthPixelLat &&
			currentTile.startLat >= lat + stats.widthPixelLat
		)
	)
		return false;
	return true;
...
function getLocationIndex() {
	for (let index in geodata) {
		...
		if (
			!(
				tile.startLong <= long + stats.widthPixelLong && tile.endLong >= long - stats.widthPixelLong
			)
		)
			continue;
		if (!(tile.endLat <= lat - stats.widthPixelLat && tile.startLat >= lat + stats.widthPixelLat))
			continue;
		return index;
	}
	return -1;
...
function getTarget(client) {
	...
	if (targetLat == Infinity || targetLong == Infinity || lat == Infinity || long == Infinity)
		message = '-1';
	else message = `${targetPixelX} ${targetPixelY}`;

	send(
		client,
		`${outgoing.target}:${message}`,
		`Zielpunkt mit dem Wert "${targetPixelX} ${targetPixelY}" wird zum Klienten gesendet.`,
		null,
		importance.LOW
	);
}
...
function calculateTarget(targetLat, targetLong) {
	...
	sendAllClients(
		`${outgoing.target}:${message}`,
		`Zielpunkt mit dem Wert "${targetPixelX} ${targetPixelY}" wird an alle Klienten gesendet.`,
		null,
		importance.LOW
	);
}
